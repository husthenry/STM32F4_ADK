...
/** @defgroup STM32F429I_DISCOVERY_IOE_Private_FunctionPrototypes
  * @{
  */ 
uint16_t IOE_TP_Read_X(void);
uint16_t IOE_TP_Read_Y(void);
uint16_t IOE_TP_Read_Z(void);
void     IOE_GPIO_Config(void);
void     IOE_I2C_Config(void);
void     IOE_DMA_Config(IOE_DMADirection_TypeDef Direction, uint8_t* buffer);
...
void IOE_GPIO_Config(void)
{
  ...
  RCC_AHB1PeriphClockCmd(IOE_I2C_SCL_GPIO_CLK | IOE_I2C_SDA_GPIO_CLK | IOE_IT_GPIO_CLK, ENABLE);
