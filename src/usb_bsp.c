...
#ifdef USE_ACCURATE_TIME 
	void BSP_SetTime(uint8_t Unit);
	void BSP_Delay(uint32_t nTime,uint8_t Unit);
...
void USB_OTG_BSP_Init(USB_OTG_CORE_HANDLE *pdev)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_OTG_HS, ENABLE);
#else
	#error Undefine USB Host interface
#endif
#endif
#endif
#endif
...
{
#if defined( USE_USB_OTG_FS ) || defined( USE_EMBEDDED_PHY )
	GPIO_InitTypeDef GPIO_InitStructure;
	...
	/*	Delay is need for stabilising the Vbus Low
		in Reset Condition, when Vbus = 1
		and Reset-button is pressed by user
	 */
#endif
